...
**Rule 17.1.2 — Two composition roots, and only they read the mode.** There are exactly two: `appModeProfileProvider` in `lib/core/mode/app_mode_profile.dart` (causes 1–4) and `surfaceStrategyProvider` in `lib/page/_shared/mode/surface_strategy_provider.dart` (cause 5). No other file may read `appModeProvider`, and no file outside them may read `GlobalConfig.remote.isActive`.
