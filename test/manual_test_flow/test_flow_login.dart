...
void main() {
  ...
  const secret = 'laxhR+c4Z61RU0yQhM22zro0Ej4XqKSjjQFY9nOQaWH/TnS91JIaVy7Ckq1eNSH1uHjFE0HthsLF7Bdt4tpTUXbteU4N7M9/RG1LIQ9AD8wAsp2zKR8KZE5RDRiwd6s2pgcL4SPCcEj8W4lecG1ynqS10R5sW8RiPZFjfmLtBSY=';
  ...
  group('GROUP 1 - test login prepare and login password in dev', () {
    ...
    test('STEP 1 - login prepare', () async {
      ...
      final response = await client.loginPrepare(username, id: id, secret: secret);
    ...
    test('STEP 2 - login password', () async {
      final client = DevTestableClient();
      final response = await client.loginPassword(token, 'Linksys123!', id: id, secret: secret);
    });
    ...
    test('STEP 3 - get communication methods', () async {
      ...
      final methods = List<CommunicationMethod>.from(jsonResp.map((e) => CommunicationMethod.fromJson(e)));
      method = methods.firstWhere((element) => element.method == 'EMAIL');
    });

    test('STEP 4 - auth challenge with id', () async {
      final client = DevTestableClient();
      final response = await client.authChallenge(AuthChallengeMethodId(token: token, commMethodId: method?.id ?? ''), id: id, secret: secret);
  ...
  group('GROUP 2 - test auth challenge verification and login part in dev', () {
    const token = 'C49462D6-6BBE-40E1-BF7E-612C23D08D80';
    test('STEP 5 - auth challenge verify', () async {
      const code = '8256'; // PUT received code here.
      final client = DevTestableClient();
      final response = await client.authChallengeVerify(token: token, code: code);
    });
