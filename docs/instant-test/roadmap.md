...
- **INVESTIGATE: speed test fails recurringly** (reported 2026-06-09, repeated). Download hits `https://speed.cloudflare.com/__down?bytes=10000000` (10MB, 30s timeout) with a smaller-payload fallback; returns 0 on failure. Likely cause: CORS from the `192.168.1.1` origin, or the 10MB download timing out on the test connection. Needs live network/console inspection (browser blocked by self-signed cert for Playwright). Options: smaller payload, different/again-retry, or a router-side (USP) speed source. Root-cause separate from the UX fix below.
