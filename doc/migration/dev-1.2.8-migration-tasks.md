...
- [ ] **1.1.10**: 整合 PWA banner 至 top_bar.dart
  - 檢視 dev-1.2.8 的整合方式
...
- [ ] **1.4.1**: 檢視 SPNM62/M62 變更
  ```bash
  git show 7dae63b6
  ```

- [ ] **1.4.2**: 在對應設定檔案中啟用 SPNM62/M62
...
- [ ] 提交階段一變更
  ```bash
  git add .
  git commit -m "feat: migrate PWA, brand assets, and utility functions from dev-1.2.8"
  ```
...
- [ ] **2.3.5**: 更新 props（Equatable）
  - 加入 servers 和 selectedServer

- [ ] **2.3.6**: 更新 copyWith 方法

- [ ] **2.3.7**: 更新 toMap/fromMap 方法
...
- [ ] 提交階段二變更
  ```bash
  git add .
  git commit -m "feat: integrate speed test server selection from dev-1.2.8"
  ```
...
- [ ] 建立 Pull Request
  ```bash
  git push -u origin feature/merge-1.2.8-features
