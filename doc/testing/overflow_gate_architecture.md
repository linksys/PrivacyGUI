...
member is abstract, so it cannot be *omitted*; #1332's own ticket text read that as
the invariant being enforced, and it enforces only the declaration. What the mutation
...
family's business". Filed as its own row rather than smoothed over, which is what
#1348's last criterion asks for.
