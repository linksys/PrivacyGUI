...
A baseline is 15,549 sorted rows across five files. `check` answers "did it move";
...
**The seventh `page` capture, and it is not a wave** (#1489, 2026-09-09): 10,062 →
**10,530**, **+468 rows in two new groups, `page.statistics_devices` and
`page.statistics_system`, every one `clean`, none removed and none changed**. It is the
first capture in this dataset's history that added **no page**. The statistics page is
three-tabbed and the sweep had only ever built tab 0, for a reason its case doc stated as
fact and had inherited from a page where it is true — `UspStatisticsView` takes an
`initialTab`, so the other two tabs need no tap and never did (architecture doc §11.13).
Two cases now construct them. The 468 cells are `clean` only because the 35 that were not
were fixed in `lib/` first, under #1488, before this capture was taken; had it been taken
first it would have frozen 35 red coordinates into the dataset, which is the ordering rule
§8 of the architecture doc states and this is its first real exercise. Consequence for the
counts: **43 pages, 45 groups** — the two stopped being one number here, and the roster
still holds its 45 rows, 43 of them swept, because it keys on the page view file rather than
on the case.

**The eighth capture, same ticket and same PR** (#1489's follow-up, 2026-09-09): 10,530 →
**11,232**, **+702 rows in three new groups, `page.wifi_settings_advanced`,
`page.port_range` and `page.port_triggering`, every one `clean`, none removed and none
changed**. The seventh capture's own paragraph above is what prompted it: it said the
statistics case doc had inherited its "needs a tap per cell" claim from a page where it is
true, naming `wifi_settings`. That was also wrong. `UspWifiSettingsView` and
`UspPortForwardingDetailView` simply took no tab argument — four lines each — and once they
do, wired from `?tab=N` so it is a deep link and not a test hook, the tabs are reachable by
construction like statistics'. Read the two captures together as one finding: **the blocker
was never a second test axis, it was a missing parameter**, and the doc that said otherwise
was repeated twice before anyone checked it.

This capture cost no `lib/` layout work at all, which is the part worth recording: 702 cells
over three tabs nothing had ever laid out came back clean on the first run. That is a real
answer to "how many red cells would this add to a blocking gate" — zero — and it is only an
answer because the cells exist now. Contrast #1488, where the statistics tabs found 35.
...
| `page` | `page_surface_overflow_test.dart` | 11,934 | 0 | **51 groups × 234** over 44 pages (50 groups before #1554's second pair, 49 over the same 44 pages before its first — #1549's split is what took the page count from 43 to 44, and it took the group count from 48 with it; 45 groups before #1489's follow-up, 43 before #1489; × 208 until #1372) — `dhcp` · `wifi_settings` (#1349) · `device_list` · `device_detail` · `topology` · `node_detail` · `port_forwarding` (#1377) · `pnp_entry` · `pnp_no_internet` · `pnp_isp_settings` · `pnp_pppoe` · `pnp_static_ip` · `pnp_unplug_modem` · `pnp_modem_lights_off` · `pnp_waiting_modem` · `pnp_setup` (#1378) · `home` · `login_local` · `local_router_recovery` · `local_reset_router_password` · `menu` · `auto_parent_first_login` (#1379) · `admin` · `advanced_settings` · `apps` · `dmz` · `firewall` · `firmware_update` · `instant_privacy` · `instant_safety` · `internet_settings` · `ipv6_port_service` · `local_network` · `remote_assistance` · `router_assistant` · `sliver_dashboard` · `static_routing` · `statistics` · `support` · `system_log` · `test_console` · `unified_diagnostics` · `usp_dashboard` (#1380) · `statistics_devices` · `statistics_system` · `wifi_settings_advanced` · `port_range` · `port_triggering` (#1489, five sibling tabs of three pages already listed above) · `firmware_ota` (#1549, the 44th page, split off `firmware_update`) · `pnp_setup_firmware` (#1554, a second *fixture state* of `pnp_setup` rather than a tab — the wizard's locked firmware stage) · `firmware_failed` (#1554, the same mechanism on `firmware_update`, and the one case in this dataset that arrived because of a defect: the `failed` phase's title `Row` overflowed at 320px in four locales, unswept until it existed) |
...
identical. What is being frozen is the *coverage*: **15,549**
...
late 234 for `pnp_setup`, #1379's 1,404, #1380's 4,914, #1489's 468 + 702, #1549's 234 for
`firmware_ota` #1554's 234 for `pnp_setup_firmware` and its 234 for `firmware_failed`, **less the one** the
