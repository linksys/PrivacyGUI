...
- [ ] **Task 1.2:** Update the `FeatureState<S, T>` abstract base class to include the `copyWith` contract.
- [ ] **Task 1.3:** Create the `PreservableContract` interface and the `PreservableNotifierMixin`.
- [ ] **Task 1.4:** Update the `LinksysRoute` class to check for `PreservableContract` and call `revert()`.
...
- [ ] **Task 2.2:** Write unit tests for `Preservable<T>` and `PreservableNotifierMixin`.
- [ ] **Task 2.3:** Write an integration test for the updated `LinksysRoute` logic.

---

### **Phase 3: Reference Implementation - Refactor `InstantSafety` Feature**
- [ ] **Task 3.1:** Refactor `InstantSafetyState` to extend the updated `FeatureState`.
- [ ] **Task 3.2:** Refactor `InstantSafetyNotifier` to use `with PreservableNotifierMixin`.
- [ ] **Task 3.3:** Refactor `InstantSafetyView` to rely on the provider for state and actions.
- [ ] **Task 3.4:** Ensure the `LinksysRoute` for `/instant-safety` is correctly configured.
