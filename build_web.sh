function buildWebApp() {
  ...
  if [ "$cloud" == "qa" ]; then
    flutter build web --profile --dart-define=Dart2jsOptimization=O0 --target=lib/main.dart --base-href="/${href}" --build-number="${buildNumber}" --dart-define=force="${force}" --dart-define=cloud_env="${cloud}" --dart-define=enable_env_picker="${picker}" --web-renderer html --no-tree-shake-icons
  else
