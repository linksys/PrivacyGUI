...
class UspTopologyService {
  ...
  Map<String, dynamic> _mapDevices(UspGetResponse response) {
    UspLogger.logMappingStart('GetDevices');

    final values = _flattenResults(response);
    UspLogger.logTr181Data(values);
    ...
    final hostCount = int.tryParse(
        ...
        0;
    UspLogger.logCount('client hosts', hostCount);
    ...
    UspLogger.logMappingResult(
      'devices',
      devices
          .map((d) => (
                d['friendlyName']?.toString() ?? 'Unknown',
                d['deviceID']?.toString() ?? '',
              ))
          .toList(),
    );
