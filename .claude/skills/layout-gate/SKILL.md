...
a re-export of it since #1340. **50 suites carry `layout-gate` today** (49 counted 2026-08-27, plus #1488's `stats_legend_rows_test.dart` — #1380 added twenty-one pages and no suite, because a page is a data entry; the forty-ninth arrived the same day with the `dev-2.7.0` merge, #1367's `usp_stats_panel_test.dart`, written against the retired `dashboard-card` tag and enrolled by renaming it — the fourth file to arrive that way), and most
...
| `test/page/_shared/page_surface_overflow_test.dart` | **forty-four whole pages, declared as sixty-one cases** — every page view under `lib/page/` except the two that are unreachable, so this axis is **done**: `dhcp` and `wifi_settings` (#1349's pilot), `device_list`, `device_detail`, `topology`, `node_detail`, `port_forwarding` (#1377's wave 1), the nine reachable `instant_setup` pages `pnp_entry`, `pnp_no_internet`, `pnp_isp_settings`, `pnp_pppoe`, `pnp_static_ip`, `pnp_unplug_modem`, `pnp_modem_lights_off`, `pnp_waiting_modem`, `pnp_setup` (#1378's wave 2), the six entry surfaces `home`, `login_local`, `local_router_recovery`, `local_reset_router_password`, `menu`, `auto_parent_first_login` (#1379's wave 3), and #1380's wave 4, the remaining twenty-one — `admin`, `advanced_settings`, `apps`, `dmz`, `firewall`, `firmware_update`, `instant_privacy`, `instant_safety`, `internet_settings`, `ipv6_port_service`, `local_network`, `remote_assistance`, `router_assistant`, `sliver_dashboard`, `usp_dashboard`, `static_routing`, `statistics`, `support`, `system_log`, `test_console`, `unified_diagnostics` — at **9** screen widths × 26 locales each, **14,274** cells (11,934 until #1602 added the two `WizardWifiReady` branches of `pnp_setup` and eight screens of `unified_diagnostics`, 11,232 until #1549 split the firmware page and #1554 added fixture-state cases for `pnp_setup` and `firmware_update`; 8 widths and 3,120 until #1372 added 1080; architecture doc §11.9 for why that width and why nothing was cut). **#1489 is why cells and pages are now two counts**: three pages are swept once per tab — `statistics` as three cases, `port_forwarding` as three and `wifi_settings` as two, eight cases over three page view files. Each sibling case constructs its tab by passing `initialTab:` to the same view, never by tapping. The case docs had claimed a tap per cell was unavoidable on `wifi_settings` and `port_forwarding`, and that was wrong in a way worth remembering: the blocker was that neither view took a tab argument, four lines each, not that the gate could not reach a tab. Both now take one, wired from `?tab=N`, so it is a deep-link capability and not a test hook. The statistics tabs laid out eight of that page's twenty sections for the first time and revealed 35 red cells in four of them, fixed in `lib/` under #1488 first (architecture doc §11.13); the other three tabs swept 702 cells at zero incidents, so this half is a coverage claim and not a fix. What every sibling case needs is a `requires` entry that only *its* tab renders, because `TabBarView` builds only the selected page — so a case that silently fell back to tab 0 must be unable to find its premise. `initialTab` is an `int` behind a `clamp`, which makes every wrong value a legal one; the premise is the only thing that notices. Do not "simplify" the per-tab cases onto one shared `requires`; `page_surface_family_test.dart` pins that each of them requires at least one type no sibling requires, and will fail. They entered in waves and one at a time inside a wave, because a page cell costs **55.0ms** in the finished file against a card's 8.8ms — it read 21.9ms while the file was half this size, which is #1380's cost finding (architecture doc §11, §11.7, §11.12). `pnp_setup` is the one that shows what "one at a time" buys: it failed 208 of 208 cells on an `AppStepper` defect that was ui_kit's, so it waited a day for v2.40.2 (`linksys/privacyGUI-UI-kit#70`) instead of entering on an allowlist. Wave 3 shows what a *premise* buys: five of its six pages have no reachable loading state at all, so the blanket `forbids: [AppLoader]` is inert on them and `requires` carries the whole guard (architecture doc §11.11) — and the sixth, `auto_parent_first_login`, was the family's one loader-is-content exemption, declared as the pinned set `kPagesWhoseLoaderIsContent` — which gained a second entry in #1554 §4, `pnp_setup_firmware`, on the same argument rather than a new one: both screens are a router being flashed with the user told not to unplug it, so the progress bar is the subject and not a stand-in. Wave 4 shows what the *ordering* costs: a remainder cannot pick the clean pages, so it needed **fourteen** overflow coordinates fixed — sixteen edits in 13 files — where the three chosen waves needed one between them (§11.12). `test/fixtures/page_roster.tsv` is the register, and it now reads **46 of 46 accounted for** — 44 swept plus `pnp_complete_view` and `speed_test_view`, both unreachable. It keys on the page view *file*, so it stays at 46 rows — 44 swept plus those two — however many cases a page has; the per-tab ms/cell figures live in its `# tabs` block and the one non-tab second case's in `# second-state` |
...
(330 today; 328 at #1371) to skip all but five: measured 2026-08-27,
those same **774** tests take **9m23s under the tag and 8m17s when the five files are
...
the day this line last said "fifth". **774 — 724 before #1489, 725 before the `dev-2.7.0` merge, 499 before #1380, 439 before #1379, 429 before `pnp_setup`, 414 before #1372, 342 after #1377, 296 before it, 277 before
#1349, 590 after #1343, 2,412 before that**: all five sweeps aggregate their locales inside one
test per coordinate, so 806 of those tests declare 16,017 cells (card 102, popup 80,
chrome 57, forced-form 37, page 610) and the other **23** are readability guards — #1349's
one group, #1377's one, and wave 4's eleven — which pump **845** trees between them and
name no cell. **18 was three short**, found by #1602: the page file measures **551** tests
against 530 cell tests, and this line had been carrying the arithmetic rather than a count. The cells are what the gate
measures; the test count is only how they are named. That merge is the only step in the
ladder above that **subtracted**: #1367 gave the KPI stats panel a per-card resolution and
the `skeleton|variant=stats` forced-form coordinate stopped existing, so 78 cells became 77
and 38 tests became 37. The clocks are the pre-merge ones and are kept — one cell of 13,677
is three orders of magnitude below the ±7% these runs drift by. **#1489 then added the only
cells this ladder has ever gained without gaining a page**: five sibling-tab cases over three
pages already swept, +50 tests and +1,170 cells. Two more single-case additions followed, and the ladder only adds up with both: #1549 split one firmware page in two (+10 tests, +234 cells — the one addition here that *did* add a page) and #1554 added two *fixture states* of pages already swept (+20, +468). **#1602 added ten** (+100, +2,340) — two fixture states of `pnp_setup` and eight screens of `unified_diagnostics`, the first additions found by joining the gate's case list against a golden suite's `states:` keys rather than by a nightly failure. So `page` reads 610 tests over 14,274 cells
and the page sweep's group count (48) stops equalling its page count (43). Same reason the
clocks are kept here too — 6.9% on a clock that drifts by 90%.
...
other 5,615 tests in the PR gate are effectively free — they finish while the page file is
...
**±7% is the drift on one box in one state, and the merge-day run is the counter-example.**
Confirming the post-merge counts (`--tags layout-gate` **2,061** green, `./run_tests.sh`
**6,063** green) took 18m01s and 16m02s — about double every figure in this section, on
672s and 702s of user CPU against 553s and 580s, with ~520s of *system* time in each. Half a
run inside the kernel is a box under memory and IO pressure, not a slower gate, and it flips
the wall-clock ordering so the gate reads longer than the suite containing it. If a reading
here disagrees with yours by 2×, check user CPU before you look for a regression.
...
what naming the five sweep files measures (**774** both ways, confirmed
...
gate still measure the same 17,889 coordinates", and a refactor that stops
...
- **A page cell costs 37.7ms**, ~4× a card cell, so pages do **not** join as a
  ...
  **#1371 replaced that criterion with a measured one** (§11.10): the page file may
  cost up to the clock the gate has *without* any page cells — 134.8s against 149.8s
  at twenty-two pages, 15.0s of headroom — because below that ceiling `flutter test` overlaps it with the
