...
those same **744** tests take **9m23s under the tag and 8m17s when the five files are
...
the day this line last said "fifth". **744 — 724 before #1489, 725 before the `dev-2.7.0` merge, 499 before #1380, 439 before #1379, 429 before `pnp_setup`, 414 before #1372, 342 after #1377, 296 before it, 277 before
#1349, 590 after #1343, 2,412 before that**: all five sweeps aggregate their locales inside one
test per coordinate, so 726 of those tests declare 14,145 cells (card 102, popup 80,
chrome 57, forced-form 37, page 450) and the other **18** are readability guards — #1349's
...
cells this ladder has ever gained without gaining a page**: two more `statistics` cases, one
per tab, +20 tests and +468 cells, so `page` reads 450 tests over 10,530 cells and the page
sweep's group count (45) stops equalling its page count (43). Same reason the clocks are kept
here too.
...
what naming the five sweep files measures (**744** both ways, confirmed
...
gate still measure the same 14,145 coordinates", and a refactor that stops
