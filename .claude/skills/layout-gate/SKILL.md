...
| `test/page/_shared/page_surface_overflow_test.dart` | seven **whole pages** — `dhcp` and `wifi_settings` (#1349's pilot) plus `device_list`, `device_detail`, `topology`, `node_detail`, `port_forwarding` (#1377's wave 1) — at 8 screen widths × 26 locales each, 1,456 cells. Seven by decision, not as a class: a page cell costs 27.5ms against a card's 8.8ms, so pages enter in waves and one at a time inside a wave (architecture doc §11, §11.7). `test/fixtures/page_roster.tsv` is the register of the other 38 |
...
measured 2026-08-26,
those same **342** tests take **1m55s under the tag and 49s when the five files are
named** (`flutter test`'s own clock; the shell sees 2m11s and 55s, the difference
...
the day this line last said "fifth". **342 — 296 before #1377, 277 before #1349, 590 after
#1343, 2,412 before it**: all five sweeps aggregate their locales inside one
test per coordinate, so 340 of those tests declare 5,072 cells (card 102, popup 80,
chrome 57, forced-form 38, page 63) and the other two are readability guards — #1349's
...
what naming the five sweep files measures (**342** both ways, confirmed
...
gate still measure the same 5,072 coordinates", and a refactor that stops
