...
### Requirement: DHCP Reservations State Management
The system SHALL manage the DHCP Reservations settings using the standardized dirty-guard framework, ensuring automatic detection of unsaved changes and consistent handling of navigation.

#### Scenario: Modify DHCP Reservations settings and save
- **GIVEN** the user is on the DHCP Reservations page.
- **WHEN** the user modifies a setting (e.g., adds a new reservation).
...
- **GIVEN** the user has modified a setting on the DHCP Reservations page, making it "dirty".
