...
void main() async {
  ...
  _uspDemoDependencySetup(themeConfig: themeConfig);

  runApp(
    ProviderScope(
      overrides: _buildUspOverrides(),
...
List<Override> _buildUspOverrides() {
...
void _uspDemoDependencySetup({ThemeJsonConfig? themeConfig}) {
  // Register USP-specific service helper
  if (!getIt.isRegistered<ServiceHelper>()) {
    getIt.registerSingleton<ServiceHelper>(_UspDemoServiceHelper());
  }
...
class _UspDemoServiceHelper extends ServiceHelper {
  @override
  bool isSupportGuestNetwork([List<String>? services]) => true;
  ...
  @override
  bool isSupportHealthCheck([List<String>? services]) => false;
  ...
  @override
  bool isSupportMLO([List<String>? services]) => true;

  @override
  bool isSupportDFS([List<String>? services]) => true;
