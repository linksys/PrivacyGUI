...
final sseBootstrapProvider = FutureProvider<void>((ref) async {
  ...
  try {
    ...
    logger.d('[SSE Bootstrap] Bridge health check passed');
  } catch (e) {
    logger.w('[SSE Bootstrap] Bridge health check failed: $e — continuing');
  }
  ...
  // Timeout: bridge may be busy serving dashboard requests concurrently.
  try {
    final purged = await usp
        .purgeAllSubscriptions()
        .timeout(const Duration(seconds: 10));
    if (purged > 0) {
      logger.d('[SSE Bootstrap] Purged $purged stale OBUSPA subscriptions');
      // Let bridge stabilize after purge before opening SSE
  ...
  // Step 2: Set core subscriptions — they will be registered from the
  // onConnected callback after the first heartbeat, reducing the HTTP
  // request burst on the embedded router during bootstrap.
  // See subscriptions.g.dart (generated by usp-codegen).
  manager.setCoreSubscriptions(coreSubscriptions);

  // Step 3: Connect SSE — subscriptions fire from onConnected callback
  await manager.connect();

  logger.d('[SSE Bootstrap] Complete — SSE connected, '
      'core subscriptions will register on first heartbeat');
});
