  ...
  Widget build(BuildContext context) {
    ...
    return AppDialog(
      ...
      actions: [
        ...
        //
        // Never null, in either surface. This dialog would have survived an empty
        // arm — the Retry button below is ungated, so the action list never
        // empties, which is what saved it from the defect its sibling had — but an
        // RA operator watching a firmware upgrade that has stopped responding
        // needs a way to release the Guardian session, and "wait or retry forever"
        // is not one.
        //
        // The two widgets carry this call site's `identifier:` strings verbatim.
        // They are harvested by reading Dart source *text* into the e2e suite's
        // identifier list, so they have to stay inline literals *somewhere* —
        // `lib/components/session/session_exit_actions.dart` is now that somewhere,
        // which is also what puts the login-page label outside `lib/page/`
        // (acceptance 5c).
        ref.watch(surfaceStrategyProvider).sessionExitAction(),
        AppButton.primary(
          label:
